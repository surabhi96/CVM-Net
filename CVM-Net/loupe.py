...
class NetVLAD(PoolingBaseModel):
    ...
    def forward(self, input, do_dim_reduction=False):
        ...
        # normalize along the depth or the feature space
        # For VLAD and NetVLAD we add an additional descriptor-wise
        # L2-normalization layer after conv5. 
        input = tf.nn.l2_normalize(input, dim=3, name='l2_normalisation')
        # convert to N x D, D x N rather
        reshaped_input = tf.reshape(input, [-1, self.feature_size])
        # the weights include centroid because w = 2*alpha*c_k
        ...
        if self.add_batch_norm:
          ...
          # initialize the cluster bias ; bias = -alpha*L2_norm(c_k)
        ...
        # apply softmax ~ second module in soft assignment
        # this gives mod a_k
        activation = tf.nn.softmax(activation)
        ####### reshape it to N x K ?
        activation = tf.reshape(activation,
                [-1, self.max_samples, self.cluster_size])
        # sum along the -2 th dimension
        ...
        # you have to get (xi - ck)
        vlad = tf.subtract(vlad,a)

        # L2 normalize column wise (intra - normalization)
        vlad = tf.nn.l2_normalize(vlad,1)
        # this is a 1d vector of size 1 x (K*D)
        vlad = tf.reshape(vlad,[-1, self.cluster_size*self.feature_size])
        # inter-normalization ~ final module in netvlad layer
        vlad = tf.nn.l2_normalize(vlad,1)
